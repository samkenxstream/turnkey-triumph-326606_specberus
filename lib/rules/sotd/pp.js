...
function buildWanted(groups, sr, publishedByWgOnly, ppLink) {
    const { config } = sr;
    let wanted;
    const result = {};
    ...
    if (isPP2002)
        ...
        if (!isRecTrack || !publishedByWgOnly)
...
exports.check = async function (sr, done) {
    ...
    const groupTypes = await sr.getDelivererGroups();
    // Check if the document is published by WGs only. If IG/AB/TAG(s) are evolved, the document cannot be on REC-track and should have sentence meaning 'pp not apply'.
    const publishedByWgOnly = groupTypes.every(groupType => groupType === 'wg');
